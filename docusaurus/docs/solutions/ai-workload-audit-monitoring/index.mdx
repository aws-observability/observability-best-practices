...
For detailed configuration, PII masking, and dashboard setup, see [GenAI Observability on AWS](../genai-observability/).
...
For detailed enablement steps, see [GenAI Observability on AWS — Enabling Observability for Agentic Workloads](../genai-observability/) and the [AgentCore Observability Quickstart](https://aws.github.io/bedrock-agentcore-starter-toolkit/user-guide/observability/quickstart.html#getting-started-with-agentcore-observability).
...
<Tabs>
  ...
  **Row 1** — High-level health and key counters.
  ...
  | Total Bedrock Spend | Single value (Logs) | Model Invocation Log — total cost query ([FinOps dashboard](../genai-observability/)) |
  ...
  **Row 5** — Model Performance
  ...
  | Widget | Widget Type | Data Source |
  |---|---|---|
  | Completion Rate (ok vs truncated) | Time series (Logs) | Model Invocation Log — DevOps query ([DevOps dashboard](../genai-observability/)) |
...
:::note
Rows 5-7 use queries from the companion [Custom Dashboards for GenAI Telemetry](../genai-observability/) guide. Use those queries directly rather than duplicating them here.
:::
...
- [GenAI Observability on AWS](../genai-observability/) — Deep dive on Bedrock Model Invocation Logging and Agent Telemetry (ADOT SDK) pipelines
- [Custom Dashboards for GenAI Telemetry](../genai-observability/) — Persona-based dashboards (DevOps, FinOps) with ready-to-use queries
