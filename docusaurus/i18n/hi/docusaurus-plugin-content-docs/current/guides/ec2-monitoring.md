# EC2 मॉनिटरिंग और Observability

## परिचय

निरंतर मॉनिटरिंग और Observability चपलता बढ़ाती है, ग्राहक अनुभव में सुधार करती है और क्लाउड वातावरण के जोखिम को कम करती है। Wikipedia के अनुसार, [Observability](https://en.wikipedia.org/wiki/Observability) इस बात का माप है कि किसी सिस्टम की बाहरी आउटपुट के ज्ञान से आंतरिक स्थितियों का कितनी अच्छी तरह अनुमान लगाया जा सकता है। Observability शब्द स्वयं नियंत्रण सिद्धांत के क्षेत्र से उत्पन्न हुआ है, जहां इसका मूल अर्थ है कि आप सिस्टम द्वारा उत्पादित बाहरी सिग्नल/आउटपुट के बारे में जानकर कंपोनेंट्स की आंतरिक स्थिति का अनुमान लगा सकते हैं।

मॉनिटरिंग और Observability के बीच अंतर यह है कि मॉनिटरिंग आपको बताती है कि सिस्टम काम कर रहा है या नहीं, जबकि Observability आपको बताता है कि सिस्टम क्यों काम नहीं कर रहा। मॉनिटरिंग आमतौर पर एक प्रतिक्रियात्मक उपाय है जबकि Observability का लक्ष्य सक्रिय तरीके से आपके Key Performance Indicators में सुधार करने में सक्षम होना है। किसी सिस्टम को तब तक नियंत्रित या अनुकूलित नहीं किया जा सकता जब तक उसे observe नहीं किया जाता। मेट्रिक्स, लॉग्स या ट्रेसेस के संग्रह के माध्यम से वर्कलोड को इंस्ट्रूमेंट करना और सही मॉनिटरिंग और observability उपकरणों का उपयोग करके सार्थक अंतर्दृष्टि और विस्तृत संदर्भ प्राप्त करना ग्राहकों को वातावरण को नियंत्रित और अनुकूलित करने में मदद करता है।

![three-pillars](../images/three-pillars.png)

AWS ग्राहकों को मॉनिटरिंग से observability में परिवर्तन करने में सक्षम बनाता है ताकि उनके पास पूर्ण एंड-टू-एंड सेवा दृश्यता हो सके। इस लेख में हम Amazon Elastic Compute Cloud (Amazon EC2) और AWS नेटिव और ओपन-सोर्स टूल्स के माध्यम से AWS Cloud वातावरण में सेवा की मॉनिटरिंग और observability में सुधार के लिए सर्वोत्तम प्रथाओं पर ध्यान केंद्रित करते हैं।

## Amazon EC2

[Amazon Elastic Compute Cloud](https://aws.amazon.com/ec2/) (Amazon EC2) Amazon Web Services (AWS) Cloud में एक अत्यधिक स्केलेबल कंप्यूट प्लेटफ़ॉर्म है। Amazon EC2 अग्रिम हार्डवेयर निवेश की आवश्यकता को समाप्त करता है, इसलिए ग्राहक केवल जो उपयोग करते हैं उसके लिए भुगतान करते हुए अनुप्रयोगों को तेज़ी से विकसित और डिप्लॉय कर सकते हैं। EC2 द्वारा प्रदान की जाने वाली कुछ प्रमुख सुविधाएं हैं Instances नामक वर्चुअल कंप्यूटिंग वातावरण, Amazon Machine Images नामक Instances के पूर्व-कॉन्फ़िगर किए गए टेम्पलेट, और Instance Types के रूप में उपलब्ध CPU, Memory, Storage और Networking क्षमता के विभिन्न कॉन्फ़िगरेशन।

## AWS नेटिव टूल्स का उपयोग करके मॉनिटरिंग और Observability

### Amazon CloudWatch

[Amazon CloudWatch](https://aws.amazon.com/cloudwatch/) एक मॉनिटरिंग और प्रबंधन सेवा है जो AWS, हाइब्रिड और ऑन-प्रेमिस अनुप्रयोगों और इंफ्रास्ट्रक्चर संसाधनों के लिए डेटा और कार्रवाई योग्य अंतर्दृष्टि प्रदान करती है। CloudWatch लॉग्स, मेट्रिक्स और इवेंट्स के रूप में मॉनिटरिंग और परिचालन डेटा एकत्र करता है। यह AWS संसाधनों, अनुप्रयोगों और AWS और ऑन-प्रेमिस सर्वर पर चलने वाली सेवाओं का एकीकृत दृश्य भी प्रदान करता है। CloudWatch आपको संसाधन उपयोग, एप्लिकेशन प्रदर्शन और परिचालन स्वास्थ्य में सिस्टम-व्यापी दृश्यता प्राप्त करने में मदद करता है।

![CloudWatch अवलोकन](../images/cloudwatch-intro.png)

### Unified CloudWatch Agent

Unified CloudWatch Agent MIT लाइसेंस के तहत एक ओपन-सोर्स सॉफ़्टवेयर है जो x86-64 और ARM64 आर्किटेक्चर का उपयोग करने वाले अधिकांश ऑपरेटिंग सिस्टम का समर्थन करता है। CloudWatch Agent Amazon EC2 Instances और हाइब्रिड वातावरण में ऑन-प्रेमिस सर्वर से ऑपरेटिंग सिस्टम में सिस्टम-स्तरीय मेट्रिक्स एकत्र करने, अनुप्रयोगों या सेवाओं से कस्टम मेट्रिक्स प्राप्त करने और Amazon EC2 instances और ऑन-प्रेमिस सर्वर से लॉग्स एकत्र करने में मदद करता है।
...
#### हाइब्रिड वातावरण में ऑन-प्रेमिस सर्वर पर CloudWatch Agent इंस्टॉल करना

हाइब्रिड ग्राहक वातावरण में, जहां सर्वर ऑन-प्रेमिस और क्लाउड दोनों में हैं, Amazon CloudWatch में एकीकृत observability प्राप्त करने के लिए समान दृष्टिकोण अपनाया जा सकता है। CloudWatch agent को सीधे Amazon S3 से या AWS Systems Manager के माध्यम से डाउनलोड किया जा सकता है। ऑन-प्रेमिस सर्वर के लिए Amazon CloudWatch को डेटा भेजने के लिए एक IAM User बनाएं। ऑन-प्रेमिस सर्वर पर Agent इंस्टॉल और स्टार्ट करें।
...
आपके Amazon EC2 Instances और आपके अनुप्रयोगों की विश्वसनीयता, उपलब्धता और प्रदर्शन बनाए रखने का एक प्रमुख पहलू [निरंतर मॉनिटरिंग](https://catalog.workshops.aws/observability/en-US/aws-native/ec2-monitoring) है। आवश्यक Amazon EC2 instances पर CloudWatch Agent इंस्टॉल होने के साथ, एक स्थिर वातावरण बनाए रखने के लिए instances के स्वास्थ्य और उनके प्रदर्शन की मॉनिटरिंग आवश्यक है। बेसलाइन के रूप में, CPU उपयोग, नेटवर्क उपयोग, डिस्क प्रदर्शन, डिस्क Reads/Writes, मेमोरी उपयोग, डिस्क स्वैप उपयोग, डिस्क स्पेस उपयोग, पेज फ़ाइल उपयोग और EC2 Instances के लॉग संग्रह जैसी चीज़ों की सिफारिश की जाती है।
...
#### स्वचालित और मैनुअल मॉनिटरिंग उपकरण

AWS दो प्रकार के उपकरण प्रदान करता है, स्वचालित और मैनुअल जो ग्राहकों को अपने Amazon EC2 की निगरानी करने और कुछ गलत होने पर वापस रिपोर्ट करने में मदद करते हैं। इनमें से कुछ उपकरणों को थोड़े कॉन्फ़िगरेशन की आवश्यकता होती है और कुछ को मैनुअल हस्तक्षेप की आवश्यकता होती है।
[स्वचालित मॉनिटरिंग उपकरणों](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/monitoring_automated_manual.html#monitoring_automated_tools) में AWS सिस्टम स्टेटस चेक, Instance स्टेटस चेक, Amazon CloudWatch alarms, Amazon EventBridge, Amazon CloudWatch Logs, CloudWatch agent, AWS Management Pack for Microsoft System Center Operations Manager शामिल हैं। [मैनुअल मॉनिटरिंग](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/monitoring_automated_manual.html#monitoring_manual_tools) उपकरणों में डैशबोर्ड शामिल हैं जिन्हें हम नीचे इस लेख में एक अलग अनुभाग में विस्तार से देखेंगे।
...
अनुप्रयोगों या सेवाओं से कस्टम मेट्रिक्स StatsD प्रोटोकॉल के साथ CloudWatch agent का उपयोग करके प्राप्त किए जा सकते हैं। StatsD एक लोकप्रिय ओपन-सोर्स समाधान है जो विभिन्न प्रकार के अनुप्रयोगों से मेट्रिक्स एकत्र कर सकता है। StatsD विशेष रूप से अपने स्वयं के मेट्रिक्स को इंस्ट्रूमेंट करने के लिए उपयोगी है, जो Linux और Windows आधारित दोनों सर्वरों का समर्थन करता है।

अनुप्रयोगों या सेवाओं से कस्टम मेट्रिक्स collectd प्रोटोकॉल के साथ CloudWatch agent का उपयोग करके भी प्राप्त किए जा सकते हैं, जो केवल Linux सर्वर पर समर्थित एक लोकप्रिय ओपन-सोर्स समाधान है जिसमें plugins हैं जो विभिन्न प्रकार के अनुप्रयोगों के लिए सिस्टम स्टैटिस्टिक्स एकत्र कर सकते हैं।
...
Amazon CloudWatch Logs ग्राहकों को मौजूदा सिस्टम, एप्लिकेशन और कस्टम लॉग फ़ाइलों का उपयोग करके निकट वास्तविक समय में सिस्टम और अनुप्रयोगों की निगरानी और समस्या निवारण करने में मदद करता है। Amazon EC2 Instances और ऑन-प्रेमिस सर्वर से CloudWatch पर लॉग्स एकत्र करने के लिए, unified CloudWatch Agent इंस्टॉल करने की आवश्यकता है।
...
एक इवेंट आपके AWS वातावरण में परिवर्तन को इंगित करता है। AWS संसाधन और अनुप्रयोग अपनी स्थिति बदलने पर इवेंट जनरेट कर सकते हैं। CloudWatch Events सिस्टम इवेंट्स की एक निकट-वास्तविक समय स्ट्रीम प्रदान करता है जो आपके AWS संसाधनों और अनुप्रयोगों में परिवर्तनों का वर्णन करती है।
...
CloudWatch ServiceLens में Resource Health एक पूर्ण प्रबंधित समाधान है जिसका उपयोग ग्राहक अपने अनुप्रयोगों में [Amazon EC2 hosts के स्वास्थ्य और प्रदर्शन](https://aws.amazon.com/blogs/mt/introducing-cloudwatch-resource-health-monitor-ec2-hosts/) को स्वचालित रूप से खोजने, प्रबंधित करने और विज़ुअलाइज़ करने के लिए कर सकते हैं।

![ec2 resource health](../images/ec2-resource-health.png)

## ओपन सोर्स टूल्स का उपयोग करके मॉनिटरिंग और Observability
...
ADOT के प्रमुख कंपोनेंट SDKs, auto-instrumentation agents, collectors और exporters हैं जो backend सेवाओं को डेटा भेजते हैं।

![adot architecture](../images/adot-arch.png)

#### ADOT Collector और Amazon CloudWatch के माध्यम से मेट्रिक्स और ट्रेसेस

AWS Distro for OpenTelemetry (ADOT) Collector को CloudWatch agent के साथ Amazon EC2 Instance पर साइड-बाय-साइड इंस्टॉल किया जा सकता है और OpenTelemetry SDKs का उपयोग Amazon EC2 Instances पर चल रहे आपके वर्कलोड से एप्लिकेशन ट्रेसेस और मेट्रिक्स एकत्र करने के लिए किया जा सकता है।
...
#### Prometheus और ADOT Collector के माध्यम से मेट्रिक्स

ग्राहक अपनी observability आवश्यकताओं के लिए एक पूर्ण ओपन-सोर्स सेटअप चुन सकते हैं।

![adot prometheus architecture](../images/adot-prom-arch.png)

#### Prometheus Node Exporter

[Prometheus Node Exporter](https://github.com/prometheus/node_exporter) क्लाउड वातावरण के लिए एक ओपन-सोर्स time series मॉनिटरिंग और अलर्टिंग सिस्टम है।
...
मॉनिटरिंग आपको सूचित रखती है कि कोई सिस्टम ठीक से काम कर रहा है या नहीं। Observability आपको समझने देती है कि सिस्टम ठीक से क्यों काम नहीं कर रहा। अच्छी observability आपको उन सवालों का जवाब देने की अनुमति देती है जिनके बारे में आपको पता नहीं था कि आपको जागरूक होने की आवश्यकता है।

आधुनिक अनुप्रयोग, जो क्लाउड में माइक्रोसर्विसेज, सर्वरलेस और असिंक्रोनस आर्किटेक्चर में चल रहे हैं, मेट्रिक्स, लॉग्स, ट्रेसेस और इवेंट्स के रूप में बड़ी मात्रा में डेटा उत्पन्न करते हैं। Amazon CloudWatch AWS Distro for OpenTelemetry, Amazon Managed Prometheus, और Amazon Managed Grafana जैसे ओपन सोर्स टूल्स के साथ, ग्राहकों को इस डेटा को एक एकीकृत प्लेटफ़ॉर्म पर एकत्र, एक्सेस और सहसंबंधित करने में सक्षम बनाता है।