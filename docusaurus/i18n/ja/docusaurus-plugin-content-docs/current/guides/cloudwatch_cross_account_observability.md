...
| **用語** | **説明** |
|------|-------------|
| **モニタリングアカウント** | 複数のソースアカウントから生成されたオブザーバビリティデータを表示および操作できる中央の AWS アカウントです |
| **ソースアカウント** | そのアカウント内に存在するリソースのオブザーバビリティデータを生成する個別の AWS アカウントです |
| **シンク (Sink)** | ソースアカウントがオブザーバビリティデータをリンクおよび共有するためのアタッチメントポイントとして機能する、モニタリングアカウント内のリソースです。各アカウントは、[AWS Region](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/CloudWatch-Unified-Cross-Account.html)[^2]ごとに 1 つの **シンク (Sink)** を持つことができます |
| **オブザーバビリティリンク** | ソースアカウントとモニタリングアカウント間に確立された接続を表し、オブザーバビリティデータの共有を促進するリソースです。リンクはソースアカウントによって管理されます。 |
...
### ステップ 1: モニタリングアカウントのセットアップ {#step-1-set-up-a-monitoring-account}
...
:::
    | テレメトリタイプ | 説明 | CloudWatch Cross-Account Observability の依存関係 |
    |----------------|-------------|-----------------------------------------------------|
    | Amazon CloudWatch のメトリクス | すべてのメトリクス名前空間を共有するか、サブセットにフィルタリングします | なし |
    | Amazon CloudWatch Logs のロググループ | すべてのロググループを共有するか、サブセットにフィルタリングします | なし |
    | ServiceLens および X-Ray | すべてのトレースを共有します (フィルタリングは利用できません) | ServiceLens と X-Ray のために Metrics、Logs、Traces を有効にする必要があります |
    | Amazon CloudWatch Application Insights のアプリケーション | すべてのアプリケーションを共有します (フィルタリングは利用できません) | Metrics、Logs、Traces、および Application Insights アプリケーションを有効にする必要があります |
    | CloudWatch Internet Monitor のモニター | すべてのモニターを共有します (フィルタリングは利用できません) | Metrics、Logs、および Internet Monitor - Monitors を有効にする必要があります |
...
### ステップ 2: ソースアカウントをリンクする {#step-2-link-the-source-accounts}
...
## クロスアカウントのテレメトリーデータのクエリ {#querying-cross-account-telemetry-data}
...
[メトリクス](#metrics)の前のステップから続けて、「Graphed metrics」を選択してから「Create Alarm」を選択することで、特定のメトリクスのアラームを作成できます。
