...
- Kubernetes デプロイメントには、Flux や ArgoCD などのツールを使用して [GitOps](/observability-best-practices/ja/guides/operational/gitops-with-amg/gitops-with-amg/#gitops-の概要) プラクティスを実装します。これらのツールを CloudWatch と統合して、GitOps ワークフローの同期ステータスと正常性を監視します。
