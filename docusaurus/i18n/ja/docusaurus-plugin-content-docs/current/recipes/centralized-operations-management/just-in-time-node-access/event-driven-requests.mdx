...
sidebar_position: 3
---
# EventBridge と JITNA の統合

ジャストインタイムノードアクセス (JITNA) を使用する場合、手動承認が必要なセッションアクセスリクエストが作成されると、[EventBridge](https://aws.amazon.com/eventbridge/) に 3 種類のイベントが送信されます。

1. リクエスター アクセスリクエストステータス更新
1. 承認者 アクセスリクエストステータス更新
1. JITNA アクセスリクエスト失敗

:::info
自動承認ポリシーに基づいて自動的に承認されたアクセスリクエストに対しては、イベントは発行されません。
:::

## イベントの詳細の例

### リクエスター アクセスリクエストイベント

以下は、イベントの詳細の例です。 `Requester Access Request Status Update` イベント。
...
以下は、イベントの詳細の例です。 `Approver Access Request Status Update` イベント。
...
ジャストインタイムのノードアクセスリクエストは、[Systems Manager OpsItem リソース](https://docs.aws.amazon.com/systems-manager/latest/APIReference/API_OpsItem.html)として保存されます。以下は、自動承認ポリシーによって自動的に承認されたセッションアクセスリクエストの OpsItem の詳細の例です。
...
<Tabs
    ...
    values={[
        {label: 'リクエスターイベント', value: 'requester'},
        {label: '承認者イベント', value: 'approver'},
        {label: '失敗イベント', value: 'failed'},
    ]}>
