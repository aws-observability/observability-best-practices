...
Application Performance Monitoring (APM) は、エンドポイントとワークフローの定期的なベースラインとテストのための [Synthetics](./synthetics.md) と、実際の顧客体験のための [RUM](./rum.md) で役立ちます。
...
### 原因よりも症状を最初に考える {#think-about-symptoms-first-over-causes}
...
[CloudWatch metrics](./metrics.md) では、任意の時系列データの上位 N を検索できます。
