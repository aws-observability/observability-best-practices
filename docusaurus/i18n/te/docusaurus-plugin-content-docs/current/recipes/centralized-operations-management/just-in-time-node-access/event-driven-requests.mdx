...
# JITNA తో EventBridge ఇంటిగ్రేషన్

Just-in-time node access (JITNA) ఉపయోగించినప్పుడు, manual approval అవసరమయ్యే session access requests సృష్టించబడినప్పుడు [EventBridge](https://aws.amazon.com/eventbridge/) కు మూడు రకాల events emit చేయబడతాయి:
...
:::info
మీ auto-approval policy ఆధారంగా స్వయంచాలకంగా ఆమోదించబడిన access requests కోసం events emit చేయబడవు.
:::

## ఉదాహరణ event వివరాలు

### Requester access request event

`Requester Access Request Status Update` event కోసం event వివరాల ఉదాహరణ క్రింద ఉంది.
...
`Approver Access Request Status Update` event కోసం event వివరాల ఉదాహరణ క్రింద ఉంది.
...
### ఉదాహరణ OpsItem వివరాలు

Just-in-time node access requests [Systems Manager OpsItem resource](https://docs.aws.amazon.com/systems-manager/latest/APIReference/API_OpsItem.html) గా నిల్వ చేయబడతాయి. Auto-approval policy ద్వారా స్వయంచాలకంగా ఆమోదించబడిన session access request కోసం OpsItem వివరాల ఉదాహరణ క్రింద ఉంది.
