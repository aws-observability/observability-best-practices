# 不同的检测与 Collector 配置方案

快速导航：

- [检测方法](#instrumentation-approaches)
...
**适用人群：** 应用开发人员、具有 Observability 专业知识的团队
...
- **完整的 AWS 支持** 覆盖整个 Observability 技术栈
- **自动服务发现** 和黄金信号
...
部署 ADOT SDK 后，它会自动检测您的应用程序，无需代码变更。ADOT SDK 在运行时动态注入代码到应用中，无需手动代码变更。注入的代码自动检测对支持框架的调用，为每个操作创建 spans，并跨服务传播上下文以构建完整的 trace。
...
您的应用使用 ADOT SDK 进行检测，以 OpenTelemetry 格式捕获运行时 metrics、logs 和 traces。ADOT SDK 注入 AWS 特定的 span 属性（`aws.local.service`、`aws.local.operation`、`aws.remote.service`、`aws.remote.operation` 等），这些是 App Signals Processor 所依赖的。
...
您的应用使用标准的上游 OpenTelemetry SDK 进行检测。它生成具有语义约定（`http.method`、`http.route`、`http.status_code` 等）的标准 OTEL spans。
...
您的应用使用 ADOT SDK 自动检测。它以 OpenTelemetry 格式捕获 logs 和 traces，无需任何代码变更。
...
3. **规划迁移** — 开始逐步将服务迁移到 ADOT 检测

## RED Metrics 计算总结

了解不同检测方案中 RED（Rate、Errors、Duration）metrics 的计算方式，对于选择正确的方案至关重要：
