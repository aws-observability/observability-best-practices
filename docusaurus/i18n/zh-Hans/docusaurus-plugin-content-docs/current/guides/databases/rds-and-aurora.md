...
监控是维护 Amazon RDS 和 Aurora 数据库集群可靠性、可用性和性能的关键环节。AWS 提供了多种工具来监控 Amazon RDS 和 Aurora 数据库资源的健康状况，在问题变得严重之前检测到它们，并优化性能以确保一致的用户体验。本指南提供了 Observability 最佳实践，以确保您的数据库平稳运行。
