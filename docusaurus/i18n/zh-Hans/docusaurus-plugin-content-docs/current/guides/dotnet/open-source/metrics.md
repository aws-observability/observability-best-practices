...
.NET 已将 OpenTelemetry 作为应用程序 observability 的标准，metrics 是与 traces 和日志并列的关键支柱。此集成使开发人员能够以最小的开销监控应用程序性能。
...
.NET 中的 OpenTelemetry 支持多种导出格式，其中 Prometheus 在 metrics 方面特别流行。这种灵活性允许团队在保持一致收集方法的同时与其首选的 observability 平台集成。
...
现在您的应用程序已完成 instrumentation，请使用 collector 代理（如 OpenTelemetry Collector、CloudWatch Agent 或 Fluent Bit）将 metrics 路由到您选择的 observability 后端。请参阅以下链接获取详细信息和实施指导。
